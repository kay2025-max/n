...
notj = commands.Bot(description='SELFBOT CREATED BY notj',
                           command_prefix=prefix,
                           self_bot=True,
                           intents=intents)
status_task = None

notj.remove_command('help')

notj.whitelisted_users = {}
...
async def on_ready():
      print(
        Center.XCenter(
            ...
            f"""[=]-------------------------------------------------------------------------------------------[=]
[  SELFBOT  VERSION  1  |  MADED  BY  :-  QUANNOTJ  |  LOGINED  AS  :-  {notj.user.name}  ]
[=]-------------------------------------------------------------------------------------------[=]
""",
                1,
            )
...
async def on_message(message):
    ...
    await notj.process_commands(message)
...
TOKEN = config.get('token')
notj.add_cog(StatusRotator(notj, TOKEN))
...
> 🎗️ **DEVELOPER** :- `quannotj`
"""

    elif "config" in helpcategory:
        description = """:cro: **Config Section** :cro:
...
🥂 **Set Ltc Key** :- `.setltckey <ltc_key>`"""
        ...
        description = """:cro: **General Section** :cro:

🥤 **Show Allcmds** :- `.allcmds`
🥤 **Srv Clone** :- `.csrv <copy id> <target id>`
🥤 **Emoji Clone**  — `;emoji <copy id> <target id>`
🥤 **Restart Bot** :- `.restart`
🥤 **Server Info** :- `.srvinfo`
...
🥤 **Abuse** :- `.abuse <user>`"""
        ...
        description = """:cro: **Crypto Section** :cro:

<:crypto:1348641360650960978> **Send Ltc** :- `.send <addy> <amount>`
<:crypto:1348641360650960978> **Check Balance** :- `.bal <addy>`
<:crypto:1348641360650960978> **Check Mybal** :- `.mybal`
<:crypto:1348641360650960978> **Second Check Mybal** :- `.mybal2`
<:crypto:1348641360650960978> **Ltc Addy** :- `.addy`
<:crypto:1348641360650960978> **Second Ltc Addy** :- `.addy2`
<:crypto:1348641360650960978> **Ltc Custom Qr** :- `.ltcqr <addy> <usd_amt>`
<:crypto:1348641360650960978> **Ltc Price In Usd** :- `.ltc`
<:crypto:1348641360650960978> **Sol Price In Usd** :- `.sol`
<:crypto:1348641360650960978> **Btc Price In Usd** :- `.btc`
<:crypto:1348641360650960978> **Usdt Price In Usd** :- `.usdt`
<:crypto:1348641360650960978> **Xrp Price In Usd** :- `.xrp`
"""

    elif "msg" in helpcategory:
        description = """:cro: **Message Section** :cro:
...
🍭 **Mass Dm Friends** :- `.massdmfrnds <msg>`"""
        ...
        description = """:cro: **Status Section** :cro:
...
🍷 **Remove Status** :- `.stopactivity`"""
        ...
        description = """:cro: **Calculate Section** :cro:
...
🍩 **Usd To Euro** :- `.u2e <usd amount>`"""
        ...
        description = """:cro: **Auto Sender Section** :cro:
...
🖼️ **Get Image** :- `.get_image <query>`"""
        ...
        description = """:cro: **Upi Section** :cro:
...
💸 **Custom Qr** :- `.cqr <amt> <note>`"""
        ...
        description = """:cro: **Moderation Section** :cro:
...
🛡️ **Create Role** :- `.role <role-name>`"""
        ...
        description = """:cro: **Fun Section** :cro:
...
📋 **Name** :- `.name <name>`
📋 **Fake Nitro** :- `.nitro`
📋 **Impersonate Msg** :- `.impresonate <user>`
📋 **Check Profile** :- `.checkprofile <user>`
📋 **Blurpify** :- `.blurpify <user>`
📋 **Deepfry** :- `.deepfry <user>`
📋 **Fake Captcha** :- `.captcha <user>`
📋 **Threat** :- `.threat <user>`
📋 **Fake Iphone Gift** :- `.iphone <user>`
📋 **Fake Shipping** :- `.ship <user>`"""

    elif "vc" in helpcategory:
        description = """:cro: **Vc Section** :cro:
...
🍿 **Vc Set Limit** :- `.vclimit <limit> <channel-id>`"""
        ...
        description = """:cro: **User Section** :cro:
...
🍑 **Hentai** :- `.hentai`"""
        ...
        description = """:cro: **Vouch Section** :cro:
...
@notj.command()
async def bid(ctx):
    message = (f"<:crypto:1348641360650960978> **BINANCE ID** <:crypto:1348641360650960978>")
    message2 = (f"{BID}")
    message3 = (f"**MUST SEND SCREENSHOT AFTER PAYMENT**")
    await ctx.send(message)
    await ctx.send(message2)
...
@notj.command(aliases=["streaming"])
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/https://Wallibear",
    )
    await notj.change_presence(activity=stream)
...
@notj.command(aliases=["playing"])
async def play(ctx, *, message):
    game = discord.Game(name=message)
    await notj.change_presence(activity=game)
...
@notj.command(aliases=["watch"])
async def watching(ctx, *, message):
    await notj.change_presence(activity=discord.Activity(
...
async def stopactivity(ctx):
    await ctx.message.delete()
    await notj.change_presence(activity=None, status=discord.Status.dnd)
...
async def csrv(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = notj.get_guild(source_guild_id)
    target_guild = notj.get_guild(target_guild_id)
...
@notj.command(aliases=["pay", "sendltc"])
...
async def am(ctx, time_in_sec: int, channel_id: int, *, content: str):
    channel = notj.get_channel(channel_id)
    await ctx.message.delete()
...
- VERSION > 1
- DEVELOPER > `quannotj`
- SUPPORT SERVER > ttps://dsc.gg/quannotj
- AUTOBUY LINK > https://dsc.gg/quannotj''')
    await ctx.message.delete()
...
async def nukesrv(ctx):
    ...
    message_text = '# FUCKED BY notj  :   ||@everyone||'
...
async def unban(ctx, user_id: int):
    user = await notj.fetch_user(user_id)
...
async def nitro(ctx):
    ...
    await ctx.send(f"https://discord.gift/{gift_code}")
...
@notj.command(name='vckick', aliases=['vkick'], brief="Kicks vc user", usage=".vckick <mention.user>")
...
@notj.command(name='vcmoveall', aliases=['moveall'], brief="Moves all users to another vc", usage=".vcmoveall <from.channel.id> <to.channel.id>")
async def vcmoveall(ctx, channel1_id: int, channel2_id: int):
    await ctx.message.delete()
    channel1 = notj.get_channel(channel1_id)
    channel2 = notj.get_channel(channel2_id)
...
@notj.command(name='vcmute', aliases=['stfu'], brief="Mutes a vc user", usage=".vcmute <mention.user>")
...
async def closealldms(ctx):
    await ctx.message.delete()
    dm_user_ids = []
    for dm in notj.private_channels:
...
async def get_friends():
    relationships = await notj.http.get_relationships()
...
async def leaveallgroups(ctx):
    await ctx.message.delete()
    for channel in notj.private_channels:
...
@notj.command(name="hass", description="Random hentai ass")
async def hass(ctx):
...
@notj.command(name="ass", description="Random ass")
async def ass(ctx):
...
async def allcmds(ctx):
    ...
    cmds = [command.name for command in notj.commands]
...
@notj.command()
async def emoji(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = notj.get_guild(source_guild_id)
    target_guild = notj.get_guild(target_guild_id)

    if not source_guild or not target_guild:
        await ctx.send("❌ Không tìm thấy server! Hãy chắc chắn bot có trong cả hai server.")
        return

    await ctx.send(f"🔄 Đang sao chép emoji từ **{source_guild.name}** sang **{target_guild.name}**...")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for emoji in source_guild.emojis:
            ...
            file_name = f"{emoji.name}.png"
            with open(file_name, "wb") as f:
                f.write(img_data)

            with open(file_name, "rb") as f:
                try:
                    new_emoji = await target_guild.create_custom_emoji(name=emoji.name, image=f.read())
                    count += 1
                    print(f"✅ Đã thêm: {new_emoji.name}")
                except discord.HTTPException:
                    print(f"❌ Không thể thêm {emoji.name} (server đã đầy hoặc lỗi)")

            os.remove(file_name)

    await ctx.send(f"✅ Đã sao chép **{count}** emoji thành công!")   

notj.run(token, bot=False)